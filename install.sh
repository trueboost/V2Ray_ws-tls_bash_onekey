...
web_camouflage(){
    ...
    git clone https://github.com/trueboost/3DCEList.git
    judge "web 站点伪装"
}
v2ray_install(){
...
v2ray_conf_add_tls(){
    cd /etc/v2ray
    wget https://raw.githubusercontent.com/trueboost/V2Ray_ws-tls_bash_onekey/master/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2(){
    cd /etc/v2ray
    wget https://raw.githubusercontent.com/trueboost/V2Ray_ws-tls_bash_onekey/master/http2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
update_sh(){
    ol_version=$(curl -L -s https://raw.githubusercontent.com/trueboost/V2Ray_ws-tls_bash_onekey/master/install.sh | grep "shell_version=" | head -1 |awk -F '=|"' '{print $3}')
    ...
    then
        ...
        case $update_confirm in
            [yY][eE][sS]|[yY])
                wget -N --no-check-certificate https://raw.githubusercontent.com/trueboost/V2Ray_ws-tls_bash_onekey/master/install.sh
...
menu(){
    ...
    echo -e "\t---authored by trueboost---"
    echo -e "\thttps://github.com/trueboost\n"
